...
fn test_hf_list_models() {
    ...
    println!("Testing HuggingFace hf-list-models command...");

    let output = std::process::Command::new(common::get_binary_path())
        .args(&["hf-list-models", "--page", "1", "--page-size", "10"])
        .output()
    ...
    assert!(
        ...
        "hf-list-models should succeed, but exited with: {:?}",
        output.status
    );
    ...
    assert!(
        common::contains_any(&stdout, &["Model identifiers", "models", "page 1"]),
        "Output should mention models"
    );
}

/// Test hf-list-tags command
#[test]
fn test_hf_list_tags() {
    ...
    println!("Testing HuggingFace hf-list-tags command...");

    // Use a known HuggingFace model with Ollama support
    let model = "bartowski/Llama-3.2-1B-Instruct-GGUF";

    let output = std::process::Command::new(common::get_binary_path())
        .args(&["hf-list-tags", model])
        .output()
        .expect("Failed to execute hf-list-tags command");
    ...
    assert!(
        output.status.success(),
        "hf-list-tags should succeed for valid model, but exited with: {:?}",
        output.status
    );

    // Output should contain tag information
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        common::contains_any(&stdout, &["Model tags", "tags"]),
