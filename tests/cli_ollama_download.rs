...
fn test_ollama_list_models() {
    ...
    println!("Testing Ollama list-models command...");

    let output = std::process::Command::new(common::get_binary_path())
        .args(&["list-models"])
        .output()
        .expect("Failed to execute list-models command");
    ...
    assert!(
        ...
        "list-models should succeed, but exited with: {:?}",
        output.status
    );
    ...
    assert!(
        common::contains_any(&stdout, &["Model identifiers", "models"]),
...
fn test_ollama_list_tags() {
    ...
    println!("Testing Ollama list-tags command...");

    // Use a known model
    let model = "llama3.2";

    let output = std::process::Command::new(common::get_binary_path())
        .args(&["list-tags", model])
        .output()
        .expect("Failed to execute list-tags command");
    ...
    assert!(
        output.status.success(),
        "list-tags should succeed for valid model, but exited with: {:?}",
        output.status
    );
